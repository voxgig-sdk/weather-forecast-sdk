...
// WeatherForecast SDK retry feature
...
class WeatherForecastRetryFeature extends WeatherForecastBaseFeature
{
    ...
    public function init(WeatherForecastContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (WeatherForecastContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(WeatherForecastContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(WeatherForecastContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
