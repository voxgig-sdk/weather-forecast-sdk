...
// WeatherForecast SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class WeatherForecastTestFeature extends WeatherForecastBaseFeature
{
    ...
    public function init(WeatherForecastContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (WeatherForecastContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = WeatherForecastParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (WeatherForecastContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(WeatherForecastContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = WeatherForecastParam::call($ctx, $k);
