...
class WeatherEntityTest extends TestCase
{
    ...
    {
        $testsdk = WeatherForecastSDK::test(null, null);
    ...
    public function test_stream(): void
    {
        ...
        $base = WeatherForecastSDK::test($seed, null);
        $seen = iterator_to_array($base->Weather(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = WeatherForecastConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = WeatherForecastSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set WEATHER_FORECAST_TEST_WEATHER_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function weather_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("WEATHER_FORECAST_TEST_WEATHER_ENTID");
    ...
    if ($env["WEATHER_FORECAST_TEST_LIVE"] === "TRUE") {
        ...
        $client = new WeatherForecastSDK(Helpers::to_map($merged_opts));
    }
