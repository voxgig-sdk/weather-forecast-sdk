...
// WeatherForecast SDK feature test
//
...
class FtCtrl extends WeatherForecastControl
{
...
class FtClient
{
    ...
    private WeatherForecastUtility $utility;

    public function __construct(WeatherForecastUtility $utility, array $options)
    {
    ...
    public function get_utility(): WeatherForecastUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (WeatherForecastContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public WeatherForecastContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (WeatherForecastContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = WeatherForecastConfig::make_feature($name);
    ...
    private static function buildUrl(WeatherForecastSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof WeatherForecastError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new WeatherForecastResponse($fetched);
        }
    ...
    private function fail(WeatherForecastContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (WeatherForecastContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
