...
//     (\`new WeatherForecastSDK(...)\` / \`WeatherForecastSDK.test(...)\` become
//     \`WeatherForecastSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'WeatherForecastSDK'
...
// rewrite real client construction (\`new WeatherForecastSDK(...)\` and any
// \`WeatherForecastSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+WeatherForecastSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/WeatherForecastSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
