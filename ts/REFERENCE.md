# WeatherForecast TypeScript SDK Reference

Complete API reference for the WeatherForecast TypeScript SDK.
...
```ts
new WeatherForecastSDK(options?: object)
```
...
#### `WeatherForecastSDK.test(testopts?, sdkopts?)`
...
**Returns:** `WeatherForecastSDK` instance in test mode.
...
Alias for `WeatherForecastSDK.test()`.
...
```ts
const results = await client.Weather().list({ location: "example" })
```
...
Return the parent `WeatherForecastSDK` instance.
