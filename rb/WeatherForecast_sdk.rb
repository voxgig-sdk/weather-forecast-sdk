...
require_relative 'WeatherForecast_types'
...
class WeatherForecastSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = WeatherForecastHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = WeatherForecastHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, WeatherForecastFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    WeatherForecastUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = WeatherForecastHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = WeatherForecastHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = WeatherForecastHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => WeatherForecastError.new(
        "#{op}_allow",
        "WeatherForecastSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue WeatherForecastError => err
    ...
    if fetched.is_a?(Hash)
      status = WeatherForecastHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = WeatherForecastError.new(
        "graphql_error", "WeatherForecastSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = WeatherForecastSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
