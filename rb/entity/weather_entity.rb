# WeatherForecast SDK Weather entity
...
class WeatherEntity
  ...
  def data_set(args)
    if args
      @_data = WeatherForecastHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = WeatherForecastHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Array<Weather>, Array] the matching Weather items; raises WeatherForecastError on failure
