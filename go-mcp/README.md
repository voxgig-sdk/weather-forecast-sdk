...
[MCP](https://modelcontextprotocol.io) server exposing the WeatherForecast SDK as
two agent tools — `weather-forecast_list` and `weather-forecast_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/weather-forecast-mcp)
make build

# 2. Provide credentials via the environment
export WEATHER_FORECAST_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user weather-forecast \
  -- /absolute/path/to/weather-forecast-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./weather-forecast-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// weather-forecast_list: first page of records
{ "entity": "weather" }
{ "entity": "weather", "query": { } }

// weather-forecast_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/weather-forecast-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user weather-forecast \
     -- "$PWD"/dist/*/weather-forecast-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `weather-forecast_list` and `weather-forecast_load` tools now appear
   in new sessions. Ask the agent to *"list weather using weather-forecast"*
   and it calls `weather-forecast_list` with `{"entity":"weather"}`.
...
```sh
export WEATHER_FORECAST_APIKEY=sk_live_xxx            # API key
export WEATHER_FORECAST_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `weather-forecast_list` tool
...
### Call the `weather-forecast_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `weather-forecast_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `weather-forecast_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `WEATHER_FORECAST_APIKEY` | API key sent with every request. |
| `WEATHER_FORECAST_BASE` | Optional override of the API base URL. |
...
```sh
./weather-forecast-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"weather-forecast_load","arguments":{"entity":"weather","query":{"id":1}}}}'
```
