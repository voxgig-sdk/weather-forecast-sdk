# WeatherForecast SDK utility: make_request
...
module WeatherForecastUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = WeatherForecastResponse.new({})
    ...
    if fetch_err
      ...
      response = WeatherForecastResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = WeatherForecastResponse.new(fetched)
    else
