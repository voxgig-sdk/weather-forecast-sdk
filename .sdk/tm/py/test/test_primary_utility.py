...
from projectname_sdk import WeatherForecastSDK
from projectname_sdk.core.spec import WeatherForecastSpec
from projectname_sdk.core.result import WeatherForecastResult
from projectname_sdk.core.response import WeatherForecastResponse
from projectname_sdk.core.operation import WeatherForecastOperation
from projectname_sdk.core.error import WeatherForecastError
from projectname_sdk.core import helpers
from projectname_sdk.feature.base_feature import WeatherForecastBaseFeature
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return WeatherForecastError(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_make_error_no_throw(self):
        ...
        ctx.result = WeatherForecastResult({
            "ok": False,
            "resdata": {"id": "safe01"},
        })
    ...
    def test_feature_init_basic(self):
        init_client = WeatherForecastSDK.test(None, None)
        ...
        class TestInitFeature(WeatherForecastBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "initfeat"
                self.active = True
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(WeatherForecastBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_fetcher_live(self):
        ...
        live_client = WeatherForecastSDK({
    ...
    def test_fetcher_blocked_test_mode(self):
        ...
        blocked_client = WeatherForecastSDK({
            "base": "http://localhost:8080",
            "system": {
                "fetch": mock_fetch,
            },
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = WeatherForecastSpec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = WeatherForecastSDK.test(None, setup_opts)
    ...
    def test_operator_basic(self):
        ...
        def subject(entry):
            ...
            if not isinstance(in_val, dict):
                in_val = {}
            op = WeatherForecastOperation(in_val)
            return {
                "entity": op.entity,
                "name": op.name,
                "input": op.input,
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = WeatherForecastSDK.test(None, setup_opts)
