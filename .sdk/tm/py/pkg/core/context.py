...
from projectname_sdk.core.control import WeatherForecastControl
...
class WeatherForecastContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = WeatherForecastControl()
        ...
        if isinstance(sp, WeatherForecastSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, WeatherForecastResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, WeatherForecastResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return WeatherForecastOperation({})
    ...
    def make_error(self, code, msg):
        return WeatherForecastError(code, msg, self)
