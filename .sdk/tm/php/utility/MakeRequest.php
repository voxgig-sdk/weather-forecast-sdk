...
class WeatherForecastMakeRequest
{
    public static function call(WeatherForecastContext $ctx): array
    {
        ...
        if ($fetch_err) {
            ...
            $response = new WeatherForecastResponse(['err' => $ctx->make_error('request_no_response', 'response: undefined')]);
        } elseif (is_array($fetched)) {
            $response = new WeatherForecastResponse($fetched);
        } else {
