...
// WeatherForecast SDK pipeline test
//
...
{
    private static function utility(): WeatherForecastUtility
    {
        return new WeatherForecastUtility();
    }
    ...
    private static function ctx(array $over = []): WeatherForecastContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new WeatherForecastContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new WeatherForecastOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof WeatherForecastError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new WeatherForecastOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = WeatherForecastMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new WeatherForecastOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = WeatherForecastMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new WeatherForecastOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = WeatherForecastMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new WeatherForecastSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = WeatherForecastMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        ...
        [, $err] = WeatherForecastMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new WeatherForecastSpec([]),
            'response' => null,
            'result' => new WeatherForecastResult([]),
        ]);
        [, $err] = WeatherForecastMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new WeatherForecastSpec(['step' => 's']),
            'response' => new WeatherForecastResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new WeatherForecastResult([]),
        ]);
        [, $err] = WeatherForecastMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new WeatherForecastSpec(['step' => 's']),
            'response' => new WeatherForecastResponse(self::resp(200, ['v' => 1])),
            'result' => new WeatherForecastResult([]),
        ]);
        [, $err] = WeatherForecastMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new WeatherForecastSpec(['step' => 's']),
            'response' => new WeatherForecastResponse(self::resp(200, ['v' => 2])),
            'result' => new WeatherForecastResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        WeatherForecastMakeResponse::call($ctx);
    ...
    {
        $preset = new WeatherForecastResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new WeatherForecastSpec([]),
            'response' => new WeatherForecastResponse([]),
            'result' => new WeatherForecastResult([]),
        ...
        [$got, $err] = WeatherForecastMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new WeatherForecastResult([])]);
        [, $err] = WeatherForecastMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new WeatherForecastSpec([]), 'result' => null]);
        [, $err] = WeatherForecastMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new WeatherForecastSpec(['step' => 's']),
            'result' => new WeatherForecastResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new WeatherForecastOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = WeatherForecastMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new WeatherForecastSpec(['step' => 's']),
            'result' => new WeatherForecastResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new WeatherForecastOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = WeatherForecastMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new WeatherForecastResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new WeatherForecastSpec([]),
            'result' => new WeatherForecastResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = WeatherForecastMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = WeatherForecastMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new WeatherForecastError('boom', 'boom');
        $utility->fetcher = function (WeatherForecastContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new WeatherForecastSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = WeatherForecastMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (WeatherForecastContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = WeatherForecastMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(WeatherForecastResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        WeatherForecastMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (WeatherForecastContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new WeatherForecastSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = WeatherForecastMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new WeatherForecastResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new WeatherForecastSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = WeatherForecastMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = WeatherForecastMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = WeatherForecastMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new WeatherForecastResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, WeatherForecastDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new WeatherForecastResult(['ok' => false])]);
        $this->expectException(WeatherForecastError::class);
        WeatherForecastDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new WeatherForecastResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', WeatherForecastMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        WeatherForecastMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new WeatherForecastBaseFeature();
        $b = new WeatherForecastBaseFeature();
        WeatherForecastFeatureAdd::call($ctx, $a);
        WeatherForecastFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): WeatherForecastBaseFeature
    {
        $f = new WeatherForecastBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        WeatherForecastFeatureAdd::call($ctx, self::named_feature('a'));
        WeatherForecastFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        WeatherForecastFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        WeatherForecastFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        WeatherForecastFeatureAdd::call($ctx, $replace);
        ...
        WeatherForecastFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): WeatherForecastContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new WeatherForecastSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = WeatherForecastPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        WeatherForecastPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new WeatherForecastResponse(['headers' => null]),
            'result' => new WeatherForecastResult([]),
        ]);
        WeatherForecastResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        WeatherForecastResultBody::call($ctx);
