...
class TestWeatherEntity:
    ...
    def test_should_stream(self):
        ...
        base = WeatherForecastSDK.test(seed, None)
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set WEATHER_FORECAST_TEST_WEATHER_ENTID JSON to run live")
        client = setup["client"]
...
def _weather_basic_setup(extra):
    ...
    client = WeatherForecastSDK.test(options, extra)
    ...
    _entid_env_raw = os.environ.get(
        "WEATHER_FORECAST_TEST_WEATHER_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("WEATHER_FORECAST_TEST_WEATHER_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("WEATHER_FORECAST_TEST_LIVE") == "TRUE":
        merged_opts = vs.merge([
            {
            },
            extra or {},
        ])
        client = WeatherForecastSDK(helpers.to_map(merged_opts))

    _live = env.get("WEATHER_FORECAST_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("WEATHER_FORECAST_TEST_EXPLAIN") == "TRUE",
        "live": _live,
