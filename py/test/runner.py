# WeatherForecast SDK test runner

from __future__ import annotations
import os
import json

from weatherforecast_sdk.utility.voxgig_struct import voxgig_struct as vs


class WeatherForecastTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            WeatherForecastTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = WeatherForecastTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = WeatherForecastTestRunner.getenv("WEATHER_FORECAST_TEST_LIVE")
        override = WeatherForecastTestRunner.getenv("WEATHER_FORECAST_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = WeatherForecastTestRunner.getenv(key)
        ...
        explain = WeatherForecastTestRunner.getenv("WEATHER_FORECAST_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["WEATHER_FORECAST_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if WeatherForecastTestRunner._test_control is not None:
            return WeatherForecastTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                WeatherForecastTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            WeatherForecastTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = WeatherForecastTestRunner.load_test_control()
...
def load_env_local():
    WeatherForecastTestRunner.load_env_local()


def env_override(m):
    return WeatherForecastTestRunner.env_override(m)


def entity_data(v):
    return WeatherForecastTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return WeatherForecastTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return WeatherForecastTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return WeatherForecastTestRunner.load_test_control()


def live_client_options():
    return WeatherForecastTestRunner.live_client_options()


def live_delay_ms():
    return WeatherForecastTestRunner.live_delay_ms()
