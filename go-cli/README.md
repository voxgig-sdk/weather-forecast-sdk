...
boru-driven command-line client **and** interactive REPL for the WeatherForecast
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/weather-forecast-cli)
make build

# 2. See usage (words, entities, env vars)
./weather-forecast-cli --help

# 3. Provide credentials once, via the environment
export WEATHER_FORECAST_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./weather-forecast-cli list weather

# 5. Override the API base URL for a single call
WEATHER_FORECAST_BASE=https://api.example.com ./weather-forecast-cli list weather

# 6. No arguments -> interactive REPL
./weather-forecast-cli
weather-forecast> list weather
weather-forecast> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/weather-forecast-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/weather-forecast-cli list weather
   ```
...
Run with no arguments to open a REPL (prompt `weather-forecast>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/weather-forecast-cli`. |
...
(`./weather-forecast-cli <expr>`) and interactively (the REPL), and expressions compose the
