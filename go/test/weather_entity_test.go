...
func TestWeatherEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set WEATHERFORECAST_TEST_WEATHER_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func weatherBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("WEATHERFORECAST_TEST_WEATHER_ENTID")
	...
	idmapResolved := core.ToMapAny(env["WEATHERFORECAST_TEST_WEATHER_ENTID"])
	...
	if env["WEATHERFORECAST_TEST_LIVE"] == "TRUE" {
		mergedOpts := vs.Merge([]any{
			map[string]any{
				"apikey": env["WEATHERFORECAST_APIKEY"],
			},
			extra,
		})
		client = sdk.NewWeatherForecastSDK(core.ToMapAny(mergedOpts))
	}
